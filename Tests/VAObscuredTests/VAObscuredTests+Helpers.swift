...
extension VAObscuredTests {
    ...
    func test_Obscured_XOR() throws {
        ...
        let xored = ObscuredMacro.xor(data: expected.data(using: .utf8)!, key: key)
        let deXored = ObscuredMacro.xor(data: xored, key: key)
    ...
    func test_Obscured_XOR_Validation() throws {
        ...
        XCTAssertTrue(ObscuredMacro.getIsXORValid(result: data, key: key))
    }

    func test_Obscured_XOR_Validation_Failed() throws {
        let data: [UInt8] = [0xD8, 0x00]

        XCTAssertFalse(ObscuredMacro.getIsXORValid(result: data, key: key))
    }
}
#endif
